...
def get_pos_data():
	...
	item_list = get_items_list(pos_profile)
	final_item = get_barcode_data(item_list)

	return {
		'doc': doc,
		'default_customer': pos_profile.get('customer'),
		'items':final_item,# get_items_list(pos_profile),
...
def get_barcode_data(item_list):
	for i in range(0, len(item_list)):
		barcodes = frappe.db.sql("""select barcode from `tabItem Barcodes` where parent = %s """,(item_list[i].item_code), as_dict=1)
		if(barcodes):
			for b in range(0, len(barcodes)):
				if(barcodes[b].barcode):
					item_list[i].update({"barcode"+str(i)+str(b) : barcodes[b].barcode})
	return item_list
